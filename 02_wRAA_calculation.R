...
ncaa_guts = ncaa_guts %>% 
  select(year,division,wOBA,wOBA_Scale,wBB,wHBP,w1B,w2B,w3B,wHR,runsWin) %>% 
  rename(lgwOBA = wOBA)
...
Batters = Batters %>% 
  mutate(wRAA = get_wRAA(wOBA = wOBA, 
                         lgwOBA = lgwOBA,
                         Scale = wOBA_Scale, 
                         PA = PA)
  )
...
Batters %>% 
  arrange(-wRAA) %>% 
  select(year,Player,school,wRAA) %>% 
  top_n(100)

#acquire ncaa color data thanks to ncaahoopR
# devtools::install_github("lbenz730/ncaahoopR")
library(ncaahoopR)
colors = ncaa_colors %>% dplyr::rename(school = ncaa_name)
...
Top10 = Batters %>% 
  mutate(Player_Year = paste0(Player,"-",year)) %>% 
  arrange(-wRAA) %>% 
  select(year,Player_Year,school,primary_color,logo_url,wRAA) %>% 
  top_n(100) %>%
...
Batters = Batters %>% 
  mutate(NonAdj_BattingRuns = round(wRAA/runsWin,1))

#View the Top 10 players in Non-Adjusted Batting Runs
top100 <- Batters %>% 
  arrange(-NonAdj_BattingRuns) %>% 
  select(year,Player,school,NonAdj_BattingRuns) %>% 
  top_n(100)
